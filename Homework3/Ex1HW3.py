...
fine=501 #set density of the grid

k_grid=np.linspace(k_low, k_high, fine)

valfun=np.zeros((1,fine)) #create array of zeros to store value function
g=np.zeros((1, fine)) #create array to store indices of optimal capital levels (useful for policy function)

#Now we'll build the return matrix:

M=np.zeros((fine, fine)) #initialize the matrix
...
conv=0.000001 #convergence level

chi=np.zeros((fine, fine)) #initialize chi
progress=conv+1
